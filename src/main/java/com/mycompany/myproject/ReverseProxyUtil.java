...
/*
 * // target server vertx.createHttpServer().requestHandler(new
 * Handler<HttpServerRequest>() { public void handle(final HttpServerRequest
 * req) {
 * 
 * System.out.println("Target server processing request: " + req.uri());
 * req.response().setStatusCode(200); req.response().setChunked(true);
 * req.response().write("foo"); req.response().end(); } }).listen(8282);
 */